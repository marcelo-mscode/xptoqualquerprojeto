...
<div style="width: 100%;height: 80px;border-bottom: 1px solid #ddd;padding: 5px 28px;">
	...
	<div class="row">
		<div class="col-md-2" style="margin-bottom: 5px;padding-right: 0px;">
		    <input type="text" id="buscaEmpresaPorEmpresa" class="form-control" placeholder="Pesquisar por nome...">
	   </div>
...
<div class="col-md-8 painel estilo-painel-job" style="margin-left: -13px;width: 766px;" id="nextConsulta">
  ...
  <c:if test="${min > 0}">
  	<c:set  value="${min - 100}" var="minimoPrev" />
  </c:if>


  <c:if test="${min == 0}">
	  <c:set  value="100" var="minimoNext" />
  </c:if>		

  <c:if test="${min > 0}">
	  <c:set  value="${min + 100}" var="minimoNext" />
 </c:if> 
  
	<ul class="pager">
	  <li class="previous"><a href="listaempresa?first=${minimoPrev}&max=100" style="border-radius: 0;border-radius: 0;padding-bottom: 8px;">
	  <i class="glyphicon glyphicon glyphicon-chevron-left" style="font-size: 19px;"></i> Anterior</a></li>
	 
	  <li class="next"><a href="listaempresa?first=${minimoNext}&max=100" style="border-radius: 0;border-radius: 0;padding-bottom: 8px;">Próximo
